...
function readLocales() {
  ...
  for (const locale of i18n.LOCALES) {
    dictionaries[locale.code] = require(path.join(ROOT, 'src', 'locales', `${locale.code}.js`));
  }
...
const FACTS = {
  ...
  'src.modules': String(fs.readdirSync(path.join(ROOT, 'src')).filter((f) => f.endsWith('.js')).length),
  'views.count': String(
...
console.log(`${ORDER.length} pages écrites dans doc/site/`);
