...
The search script ships inside this skill's own directory. Every path below is written relative to the project root, so run these commands from the project root (or prefix them with the repository's absolute path) — do not assume any other working directory:
